...
public class BridgeFinder {
    ...
    private void explore(int u, int v) {
        ...
        for (Integer w : graph.get(v)) {
            ...
            if (pre[w] == -1) {
                explore(v, w);
                System.out.println(String.format("low[%d] := %d (min of low[%d] = %d and low[%d] = %d)", v,
                        Math.min(low[v], low[w]), v, low[v], w, low[w]));
                ...
                System.out.print(" ");
                System.out.println(String.format("w != u * low[%d] := %d (min of low[%d] = %d and pre[%d] = %d)", v,
                        Math.min(low[v], pre[w]), v, low[v], w, pre[w]));
