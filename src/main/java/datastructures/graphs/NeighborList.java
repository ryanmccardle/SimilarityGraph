...
public class NeighborList {
    ...
    public NeighborList retain(Set<Integer> retainNodeSet) {
        return new NeighborList(nodeIndex,
                neighbors.stream().filter(retainNodeSet::contains).collect(Collectors.toList()));
    }
    ...
    public void prioritize(Set<Integer> prioritizedNodes, Map<Integer, Integer> vertexDegree) {
        ...
        final List<Integer> last = degreeInGk <= neighbors.size() ? neighbors.subList(degreeInGk, neighbors.size())
                : new ArrayList<>();
